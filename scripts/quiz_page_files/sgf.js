...
function displayScore(numCorrect) {
	...
	if (scoreAsPercentage) {
        var userPct = _.round((numCorrect/answers.length), 2);
		var score = _.round(100 * userPct) + '%';
	} else {
		var score = numCorrect + "/" + answers.length;
...
function doAddTime(timeToAdd = 60) {
	...
	$('#time-progress').css('animation', 'none').height();
...
function getLogData() {
	...
	var logData = {
		...
		p    : typeof siteID !== 'undefined' ? siteID : 0,
...
function logTimerPlay() {
	...
	if (window.Sporcle.sendEmbedQuizEvents) {
		...
		const msg = {source: 'sporcle', event: 'end', correct, answers, time};
				...
				if (window.userID && window.numRight > 0) {
					if (!stopwatch) {
						...
						if (typeof window.bestPercent === 'undefined' || window.bestPercent < userPct) {
	                        $j('#played-tag')
								.html('Best: '+window.numRight+'/'+answers.length+' ('+userPct+'%)')
								.show();
						}
					}
					...
					addPlayToFriendScoresCache(mode, window.numRight, playTime);
